...
class Fpga1Experiment(Experiment.Experiment):
    ...
    def do_send_command_to_device(self, command):
        ...
        try:
            ...
            #- - - - - - - - - - - - - - - - Switch and Pulse Switch and MAT Command - - - - - - - - - - - - - - - - -
            elif 'Change' in command:
                print "Sending Now"
                swtype = re.search("Change(.+?) (.+?) ([-+]?\d+)", command).group(1)
                status = re.search("Change(.+?) (.+?) ([-+]?\d+)", command).group(2)
                butNo =  re.search("Change(.+?) (.+?) ([-+]?\d+)", command).group(3)
                print "swtype ", swtype ," | Status ", status ," | butNo " , butNo
                cmd2send = "G+I+"+('SW' if swtype == 'Switch' else 'PSW' if swtype == 'PSwitch' else 'MAT' if swtype == 'Keypad' else "") + butNo + "+" + ('1' if status == 'on' else '0');                
