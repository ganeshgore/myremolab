...
import es.deusto.weblab.client.experiments.fpga.ui.BaseExperimentPanel;
import es.deusto.weblab.client.lab.comm.callbacks.IResponseCommandCallback;
import es.deusto.weblab.client.lab.experiments.IBoardBaseController;

public class FpgaExperiment extends BaseExperimentPanel {
	...
	public FpgaExperiment(IConfigurationRetriever configurationRetriever,
	...
	protected IResponseCommandCallback getResponseCommandCallback(){
	    return new IResponseCommandCallback(){
			...
			public void onSuccess(ResponseCommand responseCommand) {
			    FpgaExperiment.this.processCommandSent(responseCommand);
			}
		
			@Override
			public void onFailure(CommException e) {
				FpgaExperiment.this.arduinoMessages.setText("Error raised: " + e.getMessage());
			}
	    };
	}
	...
	private void processCommandSent(ResponseCommand responseCommand) {
		if(!responseCommand.isEmpty()){
			if (responseCommand.getCommandString().contains("WEBCAMURL")){				
				FpgaExperiment.this.webcam.setUrl(responseCommand.getCommandString().split("'")[3]);	
			}
			else{
